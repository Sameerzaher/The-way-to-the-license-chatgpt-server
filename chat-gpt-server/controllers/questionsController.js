...
exports.list = (req, res) => {
  ...
  const subSubject = req.query.subSubject || null;
  const licenseType = req.query.licenseType || null;
  const filter = req.query.filter || null; // 'remaining', 'completed', 'wrong'
  ...
  // סינון לפי סטטוס שאלות (שנותרו, שהושלמו, שגויות)
  if (filter && userId) {
    console.log('Filter requested:', filter, 'for user:', userId);
    ...
    try {
      ...
      if (filter === 'wrong') {
        ...
        const wrongQuestionIds = new Set(
          userProgress.completedQuestions
            ...
            .map(cq => String(cq.questionId))
        );
        pool = pool.filter(q => wrongQuestionIds.has(String(q.id)));
        ...
        pool = pool.filter(q => completedQuestionIds.has(String(q.id)));
      ...
      console.log(`After ${filter} filtering: ${pool.length} questions`);
    } catch (error) {
      console.error('Error filtering questions by user progress:', error);
